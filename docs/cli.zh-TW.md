...
選擇 live 語意端會捨棄保留的衝突替代版本。先檢查 live 分片與復原證據，並在
初次 keep-live 決定及命令前取得針對該操作的使用者確認：

```bash
llmwikiops manifest resolve-conflict --keep-live [--json] [--pretty]
```

取得該次精確確認後，受身份綁定且可恢復的清理可以自動繼續，而且只移除記錄身份與內容仍相符的固定工件。每次操作後重新載入狀態，只有出現可觀察進展時才繼續；發生漂移就停止並要求新的決定，而不是授權覆寫。
