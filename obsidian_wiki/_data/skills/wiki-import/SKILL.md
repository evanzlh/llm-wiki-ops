...
3. **Establish tracked source authority.** Select an existing ordinary tracked
   ...
   absolute IDs and IDs containing `..`; from repository-root CWD run read-only
   `git ls-files --error-unmatch -- <Source ID>`. The manifest-tracked and
   Git-tracked states are distinct. On failure stop and require the owner to complete
   owner review, stage, and commit externally, then rerun. The framework and
   agent must not run `git add`, `git commit`, or `git push`. Continue with only
   the verified repository-relative Source ID.
