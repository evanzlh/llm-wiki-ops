...
8. **Refresh and report.** Only a successful commit/retry permits `<wiki-cli> hot status --json`; if stale, obtain `<wiki-cli> hot inputs --json --pretty`, write only its bounded requested artifact, and finish with `<wiki-cli> hot mark-current --json`. Report sessions, Source IDs, omissions, pages, and recovery. Do not push or open a pull request.
