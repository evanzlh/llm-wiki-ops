...
Only a successful `transaction commit` or `transaction retry` permits `<wiki-cli> hot status --json`; when stale run `<wiki-cli> hot inputs --json --pretty`, write only the requested tracked `hot.md` working-tree diff, then `<wiki-cli> hot mark-current --json`. The agent must not mark stale inputs current directly.
...
6. Parent runs `<wiki-cli> transaction validate <id> --json --pretty`, reviews all changes, and runs `<wiki-cli> transaction commit <id> --json --pretty` only on pass.
