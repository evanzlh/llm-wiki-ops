...
`llmwikiops transaction begin`, write a knowledge page, create a manifest
...
6. **Write final candidates.** Write only below returned `candidate_vault`.
   ...
   removals with `llmwikiops transaction delete <id> <vault-relative-page> --json --pretty`.
7. **Validate, review, commit, or recover.** Run
   ...
   `llmwikiops transaction list --json --pretty`, require one exact match,
