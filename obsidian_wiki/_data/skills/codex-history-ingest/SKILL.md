...
8. After successful commit/retry only, run `<wiki-cli> hot status --json`; if stale, run `<wiki-cli> hot inputs --json --pretty`, write only its bounded requested artifact, then `<wiki-cli> hot mark-current --json`. Report sessions, snapshots, omissions, and pages. Do not push or open a pull request.
