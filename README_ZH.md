...
`transaction begin` 会冻结所选来源的哈希；如果来源在候选内容准备期间发生变化，commit 会失败并要求重新开始事务。若 manifest 冲突留下固定恢复日志，所有者应先检查 live 分片和工作树 diff，再通过 `obsidian-wiki manifest resolve-conflict --keep-live` 明确保留当前版本。该命令只删除身份和内容仍与日志记录一致的恢复工件。
