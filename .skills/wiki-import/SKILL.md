...
3. Write candidate replacements and new pages only below `candidate_vault`. New candidates cite relevant accepted import snapshot IDs. For every updated or merged candidate `sources`, preserve every existing Source ID that still supports retained content, add relevant import snapshot IDs, deduplicate, and keep a non-empty subset of the frozen transaction source closure. Represent approved candidate replacements and deletions explicitly, using `obsidian-wiki transaction delete <id> <vault-relative-page> --json --pretty` for each deletion.
